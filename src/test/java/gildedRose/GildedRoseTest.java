...
public class GildedRoseTest {
    ...
    @Test
    public void should_return_0_when_given_item_with_Backstage_passes_to_a_TAFKAL80ETC_concert_name_and_sellIn_is_0() {
        // given
    ...
    @Test
    public void should_return_0_when_given_item_with_Aged_Brie_name_and_sellIn_is_f1() {
        // given
        Item[] items = {new Item("Aged Brie", 10, -1)};
        GildedRose gildedRose = new GildedRose(items);
    ...
    @Test
    public void should_return_38_when_given_item_with_general_name_and_sellIn_is_0() {
        // given
        Item[] items = {new Item("test name", 0, 40)};
        GildedRose gildedRose = new GildedRose(items);

        gildedRose.updateQuality();

        Assert.assertEquals(38, items[0].getQuality());
        Assert.assertEquals(-1, items[0].getSellIn());
    }

    @Test
    public void should_return_40_when_given_item_with_Sulfuras_Hand_of_Ragnaros_name_and_sellIn_is_f1() {
        // given
        Item[] items = {new Item("Sulfuras, Hand of Ragnaros", -1, 40)};
        GildedRose gildedRose = new GildedRose(items);

        gildedRose.updateQuality();

        Assert.assertEquals(40, items[0].getQuality());
        Assert.assertEquals(-1, items[0].getSellIn());
    }
